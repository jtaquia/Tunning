...
st.set_page_config(page_title='Optimización de Hyperparametros para modelos de clasificación',
    layout='wide')

#---------------------------------#
st.write("""
# Ingeniería predictiva::Optimización de Hyperparametros para modelos de clasificación
**(Regression Edition)**

En esta implementación se utiliza un Modelo de regresion utilizando algoritmos **Random Forest**.
...
uploaded_file = st.sidebar.file_uploader("Subir un archivo CSV file", type=["csv"])
st.sidebar.markdown("""
...
st.sidebar.subheader('Parámetros de Aprendizaje')
parameter_n_estimators = st.sidebar.slider('Número de estimadores (n_estimators)', 0, 500, (10,50), 50)
...
st.sidebar.write('---')
parameter_min_samples_split = st.sidebar.slider('Mínima cantidad de muestras que se requiere  para dividir en nodos internos (min_samples_split)', 1, 10, 2, 1)
...
def build_model(df):
    ...
    st.markdown('Se esta construyendo el modelo **Y** variable:')
    st.info(Y.name)
